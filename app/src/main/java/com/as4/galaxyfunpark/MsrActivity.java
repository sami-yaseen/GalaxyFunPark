...
public class MsrActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		int extra = intent.getIntExtra(view_main.EXTRA_NAME_MSR_MODE, 0);
		...
		filter.addAction(view_main.ACTION_GET_MSR_TRACK_DATA);
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case R.id.button1:
			view_main.mBixolonPrinter.setMsrReaderMode();
			break;
			
		case R.id.button2:
			view_main.mBixolonPrinter.cancelMsrReaderMode();
			break;
		}
	}
	...
	BroadcastReceiver mReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(view_main.ACTION_GET_MSR_TRACK_DATA)) {
				Bundle bundle = intent.getBundleExtra(view_main.EXTRA_NAME_MSR_TRACK_DATA);
